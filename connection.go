...
func (c *Connection) input(payload []byte) error {
	...
	if c.state == StateWaitClientInitial {
		...
		// Now check the size.
		if len(payload) < kMinimumClientInitialLength {
	...
	if el.recvd.initialized() {
		packetNumber = c.expandPacketNumber(el, hdr.PacketNumber, int(hdr.PacketNumber__length()))
	...
	plaintext, err := el.recvCipher.aead.Open(nil, c.packetNonce(packetNumber), payload[hdrlen:], payload[:hdrlen])
	if err != nil {
